...
//                       >
//                        {` Case ID: ${project?.case_id ? (project?.case_id).split('-').pop() : 'ID Not Found'}`}
//                       </Typography>
//                     </Box>
//                   </Box>
...
const MyProject = () => {
  ...
  // --- NEW: Function to run when the API inside the modal is successful ---
  const handleReviewSuccess = () => {
    refetch(); // Silently refetches the data to update the status dynamically
    setReviewModalOpen(false); // Closes the modal
  };
    ...
    <Box sx={{ bgcolor: '#F8FAFC', pt: { xs: 4, md: 8 }, pb: 8, marginTop: '50px' }}>
      <Container maxWidth="xl" sx={{ mx: 'auto', px: { xs: 2, sm: 4, md: 6 } }}>
        ...
        >
          ...
          {projects && projects.length > 0 ? (
            projects.map((project, index) => (
              <Box key={project.project_id || project._id || index}>
                <Box
                  sx={{
                    display: 'flex',
                    flexDirection: { xs: 'column', md: 'row' },
                ...
                >
                  ...
                  <Box sx={{ width: { xs: '100%', md: '15%' }, display: 'flex', justifyContent: 'flex-start' }}>
                    ...
                    {/* Checking both analyst_status and analystStatus here just in case! */}
                    <AnalystReviewBadge 
                      status={project.analyst_status || project.analystStatus || 'notRequested'} 
                      onActionClick={() => handleOpenReviewModal(project)} 
                    />
                  </Box>
