...
// const TabComponent = ({ activeTab, setActiveTab, onToggleLayout, availableTabs,getProgress  }) => {
  
//   // Filter the tabs so we only show the ones present in availableTabs
//   // If availableTabs isn't provided (e.g. on direct page reload before state syncs), fallback to showing all tabs
//   const tabsToShow = availableTabs && availableTabs.length > 0
  ...
  return (
    ...
    >
      ...
      >
        {tabsToShow.map((tab) => {
          const isActive = activeTab === tab.id;
          const progress = getProgress(tab.id) || 0;
          ...
          return (
            <Box
              ...
              sx={{
                ...
                whiteSpace: 'nowrap',
                border: '1px solid',
                borderColor: isActive ? 'transparent' : '#d5dbe4',
                bgcolor: '#fff',
                display: 'flex',
                alignItems: 'center',
                gap: 1
            ...
            >
              {/* Circular Progress next to label */}
              {progress > 0 && (
                <CircularProgress
                  variant="determinate"
                  value={progress}
                  ...
                  thickness={4}
                  sx={{
                    color: progress === 100 ? '#22C55E' : BRAND_RED,
                  }}
                />
              )}
      ...
      <Stack direction="row" spacing={2}>
        <DownloadButton />
        <IconButton
          sx={{ bgcolor: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', p: 1.5 }}
