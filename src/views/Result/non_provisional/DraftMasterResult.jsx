...
const DraftMasterResult = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // --- USING STATIC DATA ---
      const incomingDraft = STATIC_DRAFT_DATA;
      ...
      if (incomingDraft) {
        ...
        setSectionVisibility((prev) => ({
          ...
          sequence_listing:
            prev.sequence_listing || !!incomingDraft.sequence_listing,
          industrial_applicability:
            prev.industrial_applicability ||
            !!incomingDraft.industrial_applicability,
          custom_paragraphs:
            prev.custom_paragraphs || !!incomingDraft.custom_paragraphs,
        }));
        ...
        if (incomingDraft.status === "Generating") {
          setGeneratingSections((prev) => ({
            ...
            flow_chart:
              !!incomingDraft.brief_description && !incomingDraft.flow_chart,
            block_diagram:
              !!incomingDraft.brief_description && !incomingDraft.block_diagram,
            detailed_descriptions:
              !!incomingDraft.brief_description &&
              !incomingDraft.detailed_descriptions,
          }));
        }
      } else {
        ...
        setDraftData((prev) => ({
          ...prev,
          title_of_invention: STATIC_INVENTION_DATA.title,
        }));
  ...
  const toggleSection = async (key) => {
    ...
    if (
      !isVisible &&
      key !== "custom_paragraphs" &&
      key !== "sequence_listing"
    ) {
  ...
  const handleDownloadDocx = async () => {
    ...
    Packer.toBlob(doc).then((blob) => {
      ...
      link.download = `Non Provisional Draft - ${getSafeFilename(
  ...
  */
  ...
  const isDependentsGenerating = dependents.some(
    (key) => generatingSections[key],
  );
          ...
          <div>
            <div className="draft-content-wrapper">
              {sectionsConfig.map((section) => {
                const isDiagram =
                  section.key === "flow_chart" ||
                  section.key === "block_diagram";
            ...
            <div className="sidebar-sticky">
              ...
              <div className="sidebar-card">
                ...
                <div className="d-flex flex-column gap-2">
                  ...
                  <button
                    className="btn-sidebar-outline mt-2"
                    onClick={() => navigate(`/provisional-draft/${projectId}`)}
                  >
                    Provisional Draft <FaArrowRight className="ms-1" />
                  </button>
