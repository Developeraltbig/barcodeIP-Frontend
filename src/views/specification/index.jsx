...
} from "../../features/userSpecificationApi.js";
...
export default function Index() {
  ...
  // RTK Query hooks
  const { data, isLoading } = useGetUserSpecificationQuery({
    page,
    limit,
    sortBy: "createdAt",
    sortOrder: "desc",
  ...
  // API Mutations
  const [addUserSpecification] =   useAddUserSpecificationMutation();
  const [editUserSpecification] = useEditUserSpecificationMutation();
  const [deleteUser] = useDeleteUserSpecificationMutation();
   ...
   const handleFormSubmit = async (payload) => {
    try {
      if (mode === "add") {
        ...
        await addUserSpecification({ data: payload }).unwrap();
        toast.success("Admin registered successfully!");
      } else {
        // payload = { email: "new value" } 
        // Calling edit API: { id, data }
        await editUserSpecification({ 
          id: selectedUser._id, 
          data: payload 
        }).unwrap();
  ...
  }, [selectedId, deleteUser]);

  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h2 style={styles.title}>User Specifications</h2>
        <button 
          style={styles.addBtn} 
          onClick={handleAdd}
